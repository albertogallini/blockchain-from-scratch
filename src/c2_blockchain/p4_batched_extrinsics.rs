...
impl Header {
	...
	fn verify_child(&self, child: &Header) -> bool {
		return hash(&self.child(child.extrinsics_root, child.state)) == hash(child);
	}
...
impl Block {
	...
	pub fn child(&self, extrinsics: Vec<u64>) -> Self {
		if extrinsics.len() > 0 {
			return Block{
				header: self.header.child(extrinsics[0], extrinsics.len() as u64),
				body: extrinsics,
			};
		}

		return Block{
			header: self.header.child(0, extrinsics.len() as u64),
			body: extrinsics,
		};
...
fn build_invalid_child_block_with_valid_header(parent: &Header) -> Block {
	let b = Block{
		header: parent.child(vec![1,2,3,4][1], 1),
		body:vec![1,2,3,4],
	};
	return b;
}
