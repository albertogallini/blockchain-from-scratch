...
impl Header {
	...
	fn child(&self, extrinsics_root: Hash, state_root: Hash) -> Self {
		...
		while phash % 5 == 0  { // re-hash until we get a value which is not a 5 multiplier for the hash and we set the consensus digest accrodingly. 
	...
	fn verify_child(&self, child: &Header) -> bool {
		return hash(&self.child(child.extrinsics_root, child.state_root)) == hash(child);
	}
	...
	fn verify_sub_chain(&self, chain: &[Header]) -> bool {
		...
		if chain_len == 1 {
			...
			return chain[0].verify_child(&chain[1]) && self.verify_sub_chain(&chain[1..]);
		}
		else{ // chain_len == 2
			return chain[0].verify_child(&chain[1]);
		}
	}
...
impl Block {
	...
	pub fn child(&self, pre_state: &State, extrinsics: Vec<u64>) -> Self {
		if extrinsics.len() > 0 {
			return Block{
				header: self.header.child(extrinsics[0], hash(pre_state)),
				body: extrinsics,
			};
		}

		return Block{
			header: self.header.child(0, hash(pre_state)),
			body: extrinsics,
		};
	}
	...
	pub fn verify_sub_chain(&self, pre_state: &State, chain: &[Block]) -> bool {
		...
		if chain_len == 1 {
			return false; // cannot verify the header.
		}
		else {
			let mut check  = true;
			
			check &= hash(pre_state)==chain[0].header.state_root;

			for i  in 0..chain_len-1 {
				let block = &chain[i];
				check &= block.header.extrinsics_root == block.body[0];
				check &= block.header.verify_child(&chain[i+1].header);
			}
			return check;
		}
	}
...
fn build_invalid_child_block_with_valid_header(parent: &Header, pre_state: &State) -> Block {
	...
	let extrinsics: Vec<u64> = vec![1,2,3,4,5];
	let invalid_extrinsics: Vec<u64>  = vec![3,4,5,6,7];
	
	let mut header =  Header { 
		parent: hash(parent), 
		height: parent.height+1, 
		extrinsics_root: hash(&extrinsics), 
		state_root: hash(pre_state), 
		consensus_digest: 10, 
	};

	let mut phash = hash(&header);
	while phash % 5 == 0  { // re-hash until we get a value which is not a 5 multiplier for the hash and we set the consensus digest accrodingly. 
		header.consensus_digest += 10;
		phash = hash(&header);
	}
