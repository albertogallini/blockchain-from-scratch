...
pub trait ForkChoice {
	...
	fn best_chain<'a, T: ForkChoice>(candidate_chains: &[&'a [Header]]) -> &'a [Header] 
		where T: 'a{
	
		if candidate_chains.len() == 0{
			return &[];
		}

		let mut best_chain = candidate_chains[0];

		for c_i in 0 .. candidate_chains.len()-1 {
			if T::first_chain_is_better(candidate_chains[c_i], candidate_chains[c_i+1]) {
				best_chain = candidate_chains[c_i];
			}
			else{
				best_chain = candidate_chains[c_i+1];
			}

			/*
			 a more elegant implementation is : 
			 candidate_chains.iter()
							 .max_by(|chain1, chain2| {
								T::first_chain_is_better(chain1, chain2)
							 })
							 .unwrap()
...
fn mine_extra_hard(block: &mut Block, threshold: u64) {
	while hash(&block.header) > threshold {
		block.header.consensus_digest += 10;
	}
}
...
impl ForkChoice for HeaviestChainRule {
	fn first_chain_is_better(chain_1: &[Header], chain_2: &[Header]) -> bool {
		 ...
		 let powc1:i64 = chain_1.iter()
		 					.map(|x|  THRESHOLD as i64 - hash(x) as i64)
							.sum();

		 let powc2:i64 = chain_2.iter()
		 					.map(|x|  THRESHOLD as i64 - hash(x) as i64)
							.sum();
		 powc1 > powc2
	}
...
fn create_fork_one_side_longer_other_side_heavier() -> (Vec<Header>, Vec<Header>, Vec<Header>) {
	...
	let h1 = g.child(1, 1);
	let h2 = h1.child(2, 2);
	let common_prefix = vec![g,h1,h2.clone()];

	let mut h3= h2.child(3,3);
	while hash(&h3) > THRESHOLD/3 {
		h3.consensus_digest += 10;
	}
	let mut h4= h3.child(4,4);
	while hash(&h4) > THRESHOLD/3 {
		h4.consensus_digest += 10;
	}
	let havier = vec![h2.clone(),h3,h4];

	let mut h3= h2.child(5,5);
	while hash(&h3) > THRESHOLD*2 {
		h3.consensus_digest += 10;
	}
	let mut h4= h3.child(6,6);
	while hash(&h4) > THRESHOLD*2 {
		h4.consensus_digest += 10;
	}
	let mut h5= h4.child(7,7);
	while hash(&h5) > THRESHOLD*2 {
		h5.consensus_digest += 10;
	}

	let longer = vec![h2,h3,h4,h5];

	(common_prefix,longer,havier)
...
fn bc_5_longest_chain() {
	...
	assert_eq!(LongestChainRule::best_chain::<LongestChainRule>(&[chain_1, chain_2]), chain_1);
}
...
fn bc_5_heaviest_chain() {
	...
	assert_eq!(HeaviestChainRule::best_chain::<HeaviestChainRule>(&[chain_1, chain_2]), chain_2);
}
...
fn bc_5_most_even_blocks() {
	...
	assert_eq!(MostBlocksWithEvenHash::best_chain::<MostBlocksWithEvenHash>(&[chain_1, chain_2]), chain_1);
}
...
fn bc_5_longest_vs_heaviest() {
	...
	assert_eq!(LongestChainRule::best_chain::<LongestChainRule>(&[&longest_chain, &pow_chain]), &longest_chain);

	let (_, longest_chain, pow_chain) = create_fork_one_side_longer_other_side_heavier();

	assert!(HeaviestChainRule::first_chain_is_better(&pow_chain, &longest_chain));

	assert_eq!(HeaviestChainRule::best_chain::<HeaviestChainRule>(&[&longest_chain, &pow_chain]), &pow_chain);
}
